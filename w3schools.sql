...
CREATE TABLE categories(
  ...
  Descriptionx  varchar(255)  NULL
) 


INSERT INTO categories (CategoryID, CategoryName, Descriptionx) VALUES
...
CREATE TABLE customers(
  ...
  Addresos varchar(255) DEFAULT NULL,
...
INSERT INTO customers (CustomerID, CustomerName, ContactName, Addresos, City, PostalCode, Country) VALUES
...
(45, 'Lets Shop', 'Jaime Yorres', '87 Polk St. Suite 5', 'San Francisco', '94117', 'USA'),
...
select concat(employees.FirstName,' ',employees.LastName) as FullName,
	   ...
	   select order_details.OrderID as OrderID,
		...
		/*10. Select customer together with orders with order id greater than 10400*/
		...
		select top(2) Price from products
		order by PRICE DESC

		select * from (select Price,
		...
		select top(5) Price from products
		order by PRICE DESC
		...
		/*13. Select customer who spend the most money and lives in Canada*/
		 ...
		 SELECT top(1)c.CustomerID, c.CustomerName, SUM(od.Quantity * p.Price) AS TotalSpending, c.Country
                  FROM orders o
                  JOIN customers c ON o.CustomerID = c.CustomerID
                  JOIN order_details od ON o.OrderID = od.OrderID
				  JOIN products p ON p.ProductID = od.ProductID
		...
		/*14. Select customer who spend the second most money*/
		  ...
		  select * from (
		  select *,DENSE_RANK() over(order by TotalSpending Desc) as DR from (
